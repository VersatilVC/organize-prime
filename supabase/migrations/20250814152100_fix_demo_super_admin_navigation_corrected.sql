-- Fix Demo Super Admin Navigation Issues (Corrected Version)
...
-- Find demo organization by name only (since domain column doesn't exist)
DO $$
DECLARE
    demo_org_id uuid;
BEGIN
    ...
    -- If no demo org found, create one
...
BEGIN
    ...
    -- Find demo org (by name since domain column doesn't exist)
    SELECT id INTO demo_org_id 
    FROM public.organizations 
    WHERE name ILIKE '%demo%' OR name ILIKE '%Demo Organization%'
    LIMIT 1;
