...
DROP FUNCTION IF EXISTS public.get_dashboard_stats(UUID, UUID, TEXT);
...
-- 4. Atomic user profile and role update function
...
DECLARE
  ...
  is_org_admin BOOLEAN;
  updated_user JSON;
  membership_updated BOOLEAN := FALSE;
BEGIN
  ...
  BEGIN
    ...
    IF p_new_role IS NOT NULL AND p_organization_id IS NOT NULL AND (is_super_admin OR is_org_admin) THEN
      ...
      GET DIAGNOSTICS membership_updated = FOUND;
    END IF;
