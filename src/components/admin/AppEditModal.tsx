...
export const AppEditModal: React.FC<AppEditModalProps> = ({
  ...
  useEffect(() => {
    if (app) {
      setFormData({
        ...
        base_price: app.base_price || 0,
        required_permissions: app.required_permissions || [],
        n8n_webhooks: app.n8n_webhooks || {},
        is_featured: app.is_featured || false,
        is_active: app.is_active || true,
        requires_approval: app.requires_approval || true
      });
    }
  }, [app]);

  // Auto-generate slug from name (but only if it's not manually edited)
  ...
  const updateAppMutation = useMutation({
    mutationFn: async (data: AppFormData) => {
      if (!app) throw new Error('No app to update');
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-1">
          URL identifier for the app (be careful when changing this)
        </p>
      </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex justify-between pt-6 border-t">
          ...
          <Button
            onClick={handleNext}
            disabled={!canProceed || updateAppMutation.isPending}
          >
